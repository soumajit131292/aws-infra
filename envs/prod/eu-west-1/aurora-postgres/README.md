...
```bash
cd aws-infra/envs/prod/eu-west-1/aurora-postgres
terraform init
terraform plan
terraform apply
```
...
- `prod/aurora/app-credentials`

Get both values:

```bash
aws secretsmanager get-secret-value \
  --secret-id prod/aurora/app-credentials \
  --query SecretString \
  --output text
```
...
```bash
SECRET_JSON="$(aws secretsmanager get-secret-value --secret-id prod/aurora/app-credentials --query SecretString --output text)"
...
```bash
DB_HOST="$(cd envs/prod/eu-west-1/aurora-postgres && terraform output -raw cluster_endpoint)"
DB_NAME="appdb"
...
```bash
cd envs/prod/eu-west-1/aurora-postgres
./scripts/create-pre-schema-snapshot.sh \
  --cluster-id prod-aurora-postgres \
  --change-id CHG-1234 \
  --region eu-west-1 \
  --wait
```
